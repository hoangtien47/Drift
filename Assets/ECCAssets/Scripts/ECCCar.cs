...
namespace EndlessCarChase
{
    ...
    public class ECCCar : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            if (chassis == null)
            {
                chassis = transform.Find(PlayerPrefs.GetString("CurrentCar", "Stock Car"));
            }

            Debug.Log("Current Chassis is : " + chassis.gameObject.name);
            RaycastHit hit;
        ...
        void Update()
        {
            ...
            if (gameController.startDelay > 0)
            {
        ...
        public void Rotate(float rotateDirection)
        {
            ...
            if (rotateDirection != 0)
            {
                ...
                chassis.localEulerAngles = new Vector3(rightAngle, Mathf.LerpAngle(chassis.localEulerAngles.y, rotateDirection * driftAngle + Mathf.Sin(Time.time * 50) * 50, Time.deltaTime), 0);//  Mathf.LerpAngle(thisTransform.Find("Base").localEulerAngles.y, rotateDirection * driftAngle, Time.deltaTime);
            ...
            else // Otherwise, if we are no longer rotating, straighten up the car and front wheels
            {
                ...
                chassis.localEulerAngles = Vector3.up * Mathf.LerpAngle(chassis.localEulerAngles.y, 0, Time.deltaTime * 5);
                if (CarAudio.isPlaying)
                    CarAudio.Stop();
